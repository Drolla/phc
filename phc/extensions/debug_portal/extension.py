...
Events. See phc.core.config._load_extensions for configure()'s call site,
phc.core.config.load_system for how on_bind is invoked once the System
(including its tasks:) is fully built, and phc.core.scheduler.Scheduler for
...
class DebugPortalInstance:
    ...
    extensions: -- see phc.core.config.load_system); on_start()/on_stop() own
    the actual AppRunner/TCPSite lifecycle, invoked by
    phc.core.scheduler.Scheduler on its own loop (start_hooks/stop_hooks); and
    ...
    def on_bind(self, system) -> None:
        """Store the fully-built phc.core.config.System -- see class
    ...
    def on_tick(self, devices: dict[str, Device]) -> None:
        ...
        phc.core.config.load_system), called once per tick after that tick's
