"""Building the `extensions:` section into configured instances.

One instance per `extensions.<name>.<instance>:` entry, each merged
against its extension.yaml and handed to that package's own configure().
"""
...
def _resolve_path_param(value, config_dir: Path, instance_label: str, name: str):
    """Resolve one `path: true` parameter against the config directory.

    So an installation's history/recovery files land independent of the
    process's working directory. As a safety net, warn if data exists at
    the process-cwd-relative interpretation of this path but not at the
    config-relative one: history that quietly starts empty, or a
...
def _merge_extension_params(descriptor: ExtensionDescriptor, instance_config: dict,
    ...
    """Merge one extension instance's params against its extension.yaml.

    Honors each parameter's `override` rule and `default` -- same
    validation shape as _merge_params, but with no module/device scope
    split. Raises ConfigError on an unrecognized, missing-but-required,
    or not-overridable parameter, naming `instance_label` (e.g.
    "logdb.house_log") in the message.
    ...
    about the parameter rather than something each extension
    re-implements."""
...
def _load_extensions(raw: dict, flat: dict[str, Device],
    ...
    multiple independently-scheduled logdb files). Each instance's
    params are merged against its extension.yaml, then handed to that
    package's configure(params, flat, instance_key, extensions_registry)
    entry point, where instance_key = "<extension_name>.<instance_name>"
    (also the sticky-log subscriber id, see Endpoint.subscribe_log).

    `extensions_registry` is this same `registry` dict, passed by
    reference and still growing as later instances are configured -- an
    extension resolving a cross-extension reference lazily (e.g. at
    HTTP-request time, well after load_system() has returned) sees the
    complete registry regardless of declaration order; one resolved
    eagerly, during another extension's own configure(), only sees
    instances configured earlier. Each returned object is registered under
    instance_key so a task action can look it up by name, and -- if it
    exposes on_tick(devices) -- is auto-collected as a Scheduler tick
    hook."""
