...
endpoints only, same rule as extensions.recovery, since a timer's whole
point is writing to its target), restores every persisted timer as a real
core.task.Task once at startup (see on_bind), and exposes the CRUD API
`extensions/web_ui`'s `kind: timers` panel drives (see
extensions/web_ui/panels.py's TimersPanel and extensions/web_ui/server.py's
timer routes) -- see core.config._load_extensions for how configure() is
invoked, and core.scheduler.Scheduler for on_tick/on_bind timing.

Timers are deliberately NOT a parallel scheduler: each becomes an ordinary
core.task.Task (via core.task.register_task), tagged "<instance_key>.<id>",
so firing, one-shot retirement, and repeat catch-up are all handled by
core.task.Task.run()/core.scheduler.Scheduler exactly like a hand-authored
...
from core.task import kill_tasks, register_task
...
def _task_spec(timer: TimerDef, instance_key: str) -> dict:
    """Build one timer's core.task Task spec (the same shape as a `tasks:`
    YAML entry -- see core.config._build_task), consumed by
    core.task.register_task. `time:` is always the timer's own literal
...
def configure(params: dict, flat: dict[str, Device], instance_key: str,
    ...
    mistakes, mirroring extensions.recovery.extension.configure. Loading
    ...
    registered into -- see core.config.load_system's on_bind docstring."""
...
class TimerInstance:
    ...
    core.task.Task. Looked up by name from extensions/web_ui's
    `kind: timers` panel (see extensions/web_ui/server.py)."""
    ...
    # ---------- lifecycle hooks (auto-collected, see core.config.load_system) ----------
    ...
    def on_bind(self, system) -> None:
        ...
        see core.config.load_system's on_bind docstring."""
    ...
    def on_tick(self, devices: dict[str, Device]) -> None:
        ...
        core.scheduler.Scheduler pass 2, task.finished) is removed here
    ...
    # ---------- CRUD API (used by extensions/web_ui's timers panel) ----------
    ...
    def update_timer(self, timer_id: int, *, time_spec: str, device: str, endpoint: str,
        ...
        (register_task replaces by tag -- see core.task.register_task) so
    ...
    def _build_timer(self, timer_id: int, time_spec: str, device: str, endpoint: str,
        ...
        targets, `time_spec`/`repeat_spec` against core.intervals, and (for
        action "set") `value` against the target endpoint's own
        Endpoint.from_text() -- the same conversion the Task will run at
        fire time (see core.device.Device.set_text), so a bad value is
