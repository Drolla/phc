...
user-programmable timers, decoupled from core.device/core.task -- a
TimerDef is a plain record of what to do and when. extensions/timer/
extension.py turns a set of these into live core.task.Task objects (via
core.task.register_task) and exposes the CRUD API a web UI panel drives.

TimerStore mirrors extensions.recovery.recovery.RecoveryStore's atomic
whole-file YAML rewrite (temp file + os.replace, tolerate a missing/corrupt
file on load) -- duplicated rather than imported, since extensions.timer
must not depend on extensions.recovery being configured, and the payload
...
class TimerDef:
    ...
    repeating every `repeat` (a core.intervals.parse_duration string, e.g.
    ...
    def tag(self, instance_key: str) -> str:
        """This timer's core.task.Task tag, e.g. "timer.house.4" -- unique
        ...
        core.task.register_task/kill_tasks)."""
...
def expired_one_shot(timer: TimerDef, now: float, catch_up: float) -> bool:
    ...
    never "expired" this way: core.intervals.parse_time's own repeat
...
class TimerStore:
    ...
    def write(self, next_id: int, timers: list[TimerDef]) -> None:
        ...
        extensions.recovery.recovery.RecoveryStore.write for the same
