...
after a crash/restart (see extensions/recovery/extension.py for the
on_start/on_tick/on_stop wiring that drives it). Whole-file rewrite, not
append -- unlike extensions.logdb.logdb.LogDb's much larger append-only
...
class RecoveryStore:
    ...
    def write(self, values: dict[str, object]) -> None:
        ...
        extensions.logdb.logdb.LogDb's own csv_path.parent.mkdir()."""
